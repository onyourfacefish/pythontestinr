...
Bank_balances <- data %>%
  group_by(bank_name) %>%
  summarize(bank_balance_total = sum(balance)) %>%
  arrange(desc(bank_balance_total))

# A dirty way to pull the highest bank by itself (I'm sure there's a better method)
highest_bank <- Bank_balances %>%
  group_by(bank_name) %>%
  summarize(top_bank_balance = max(bank_balance_total)) %>%
...
#---------------- Which bank registered the highest amount of unclaimed CAD$ relative to its number of
